...
The choice (or estimation) of a conditional distribution is equivalent to the choice (or estimation) of a conditional *Laplace transforms*:
\begin{equation}
...
Here are some properties of the Laplace transform, defined in \@ref(eq:LT):
...
\end{eqnarray*}
and since in the second term on the right-hand side $\exp(u_2   w_{2,t+1})   \textbf{1}_{\{w_{2,t+1} > 0 \}} \rightarrow 0$ when $u_2 \rightarrow -\infty$, \@ref(eq:petitlemme) is a consequence of the Lebesgue theorem.
:::
...
@zarg_2017 porpose the extended ARG process and the ARG$_0$ process. The latter is such that $\nu = 0$ and $\beta w_t$ is replaced with $\alpha + \beta w_t$,^[Zero is an absorbing state for $w_t$ if $\alpha = 0$.] i.e.:
\begin{equation}
...
library(TSModels)
W <- simul.ARG(300,mu=.5,nu=0,
               rho=.9,alpha=.1) #simul.ARG in package TSModels
plot(W,type="l")
```
:::
...
### Affine processes of order $p$ {#SubCarp}

Let us now define affine processes of order $p$:

:::{.definition #Carp name="Affine process of order p"}

A multivariate process $w_{t+1}$ is affine of order $p$ if there exist functions $a_1(.),\dots,a_p(.)$, and $b(.)$ such that:
$$
\varphi_t(u)=\mathbb{E}_t[\exp(u' w_{t+1})]=\exp[a_1(u)'w_t+\dots+a_p(u)'w_{t+1-p}+b(u)].
$$
:::

It can be seen that if $w_t$ is affine of order $p$, then $W_t = [w_t', w_{t-1}',\dots,w_{t-p+1}']'$ is affine of order $1$.^[We have:
\begin{eqnarray*}
...
Using the notation of Example \@ref(exm:ARp), it comes that an AR(p) processes satisfies \@ref(eq:extaffine) with $b_0(u) = \left(u, \; \frac{u^2}{2}\right)'$ and $\delta = (\nu,\sigma^2)' \in \mathcal{D}=\mathbb{R} \times \mathbb{R}^+$. In that case, $\delta$ (the vector of conditional mean and variance) can be replaced by ...
...
Consider Example \@ref(exm:ARGp), it comes that an ARG(p) processes satisfies \@ref(eq:extaffine), with $b_0(u) = - \nu \log(1-u\mu)$ and $\delta=\nu$. In that case, $\nu$ ($\ge 0$) can be specified for instance as a Markov chain or an ARG.
:::
...
Consider the same framework as in Section \@ref(stoch) when $y_t$ is a $n$-dimensional vector. That is, replace \@ref(eq:extaffine) with:
\begin{equation}
...
$$
where $\mathcal{S}$ is the set of symmetric positive semi-definite matrices. Vector $\delta$ can be replaced by $\left[ \begin{array}{l} z_{1,t+1}
\\ z_{2,t+1}\end{array} \right]$, where
...
Let $B_{t,h}$ denote the date-$t$ price of a nominal zero-coupon bond of maturity $h$. We have (see \@ref(eq:ihandM) and Subsection \@ref(AffineYields)):
\begin{equation}
B_{t,h} = \mathbb{E}^{\mathbb{Q}}_t \exp (-i_{t}-\dots-i_{t+h-1}),(\#eq:stdbond)
\end{equation}
...
$$
One can then price this bond by directly employing \@ref(eq:LTreverse), with $u_1 = 0$ and $u_i = - \omega$, $i = 2,\dots, H$.
...
AB$A[1:2,,] <- AB$A[1:2,,] - 1 # add terms for exp(-i_t)
...
\dfrac{q_{t+1}}{q_t}$ the log-inflation rate between dates $t$ and $t+1$. The real rate of maturity $h$ is given by (see Subsection \@ref(AffineYields)):
\begin{eqnarray*}
...
w_{t+h}].
\end{eqnarray*}
One can then price this bond by directly employing \@ref(eq:LTreverse), with $u_1 = \bar\omega$ and $u_i = \bar\omega-\omega$, $i = 2,\dots, H$.
:::
...
* If $w_t = (\log  S_t, x'_t)'$ then $F(t,h) = \mathbb{E}^{\mathbb{Q}}_t \exp(e'_1 w_{t+h})$. This can be calculated by using \@ref(eq:LTreverse) with $u_1 = e_1$, and $u_i = 0$, for $i=2,\dots,H$.

* If $w_t = (y_t, x'_t)'$ with $y_t = \log\frac{S_t}{S_{t-1}}$, then $F(t,h) = S_t \mathbb{E}^{\mathbb{Q}}_t \exp(e'_1 w_{t+1}+\dots+e'_1 w_{t+h})$. This can be calculated by using \@ref(eq:LTreverse) with $u_i = e'_1$,  $i=1,\dots,H$.
:::
...
$$
which can be calculated by \@ref(eq:LTreverse), with $u_1 = \nu$ and $u_i = -\omega$ for $i = 2,\dots,H$.
...
\end{eqnarray}
Eq. \@ref(eq:condvar) notably implies that  $\mathbb{V}ar_t(w_{t+h})$ is an affine function of $w_t$. Indeed $\Sigma(.)$ is an affine function, and the conditional expectations $\mathbb{E}_t(w_{t+h})$ are affine in $w_t$, as shown by \@ref(eq:condmean).
...
Note that the integral in \@ref(eq:DPS) is one dimensional (whatever the dimension of $w_t$). As shown in the following example, this can be exploited to price options.
...
Consider the same context as in Example \@ref(exm:ExogSTR), but with a stochastic (endogenous) short-term rate. For instance, assume that $i_{t+1} = \omega_0 + \omega'_1 w_t$. The option price then is:
\begin{eqnarray*}
...
```{r truncatedRR, echo=TRUE, fig.cap="Conditional cumulated distribution function (c.d.f.) of $\\frac{1}{H}(i_{t+1}+\\dots+i_{t+H})$.", fig.align = 'left-aligned', message=FALSE}
H  <- 10
...
fx <- outer(x,gamma,function(r,c){Im(s1[,1]*
                                       exp(-1i*r*c))/r})*dx[,1]
...
\right.
$$
The previous sequences do not dependent on $h$ and are given by \@ref(eq:auxLemmareverseMLT).
:::
...
\end{eqnarray}
Eq. \@ref(eq:condvarAppendix) notably shows that  $\mathbb{V}ar_t(w_{t+h})$ is an affine function of $w_t$. Indeed $\Sigma(.)$ is an affine function, and the conditional expectations $\mathbb{E}_t(w_{t+h})$ are affine in $w_t$, as shown by \@ref(eq:condmeanAppendix).
...
:::
:::{.proof}
Eq. \@ref(eq:condmeanAppendix) is easily deduced from \@ref(eq:VARw), using that $\mathbb{E}_t(\varepsilon_{t+k})=0$ for $k>0$.

As regards \@ref(eq:condvarAppendix):
\begin{eqnarray*}
...
\end{eqnarray*}
which leads to \@ref(eq:Truncated).
:::
