...
* New information at date $t=1,2,\dots,T$: $K$-dimensional *factor* $w_t$, or *state*.
...
:::{.example #exStockOption name="Stock option pricing"}

* Stock option pricing: $w_t = (y_t, z_t)$ with
    1. $y_t$: observable vector of (geometric) returns,
    2. $z_t$:  regime, unobservable by the econometrician.
...
* Parametric modelling:
		...
		* Equivalently, choice of  a conditional *Laplace transforms*:
$$
...
:::{.example #exBenoulli2 name="Conditionally Bernoulli process"}
...
## Some Properties of the Laplace Transform {#AffineLaplace}
...
\varphi$ and $\psi$ are respectively called  conditional **moment** and **cumulant** generating function.
...
\end{eqnarray*}
and since in the second term on the right-hand side $\exp(u_2   w_{2,t+1})   \textbf{1}_{\{w_{2,t+1} > 0 \}} \rightarrow 0$ when $u_2 \rightarrow -\infty$, relation \@ref(eq:petitlemme) is a consequence of the Lebesgue theorem.
:::
...
* *Car* stands for *Compound auto-regressive*.
...
\mu)\end{array} \right], u <  \dfrac{1}{\mu}$.
...
[Web-interface illustration ("ARG" panel)](https://jrenne.shinyapps.io/Affine/)
...
[Web-interface illustration ("Compound Poisson" panel)](https://jrenne.shinyapps.io/Affine/)
...
## Affine (or Car) processes {#CarProcesses}


:::{.definition #Carp name="Affine process of order p"}

A multivariate process $w_{t+1}$ is affine of order $p$ [or $Car(p)$] if
$$
...
:::{.definition name="Univariate Index Affine of order p"}
...
## Affine (or Car) processes, Markov Chains {#Markov}
...
[Web-interface illustration ("Markov-Switching" panel)](https://jrenne.shinyapps.io/Affine/)


## AFFINE (OR Car) PROCESSES, Stochastic parameters univariate affine processes {#stoch}
...
* $\delta$ (vector of conditional mean and variance) can be replaced by ...

* $\left( \begin{array}{l} z_{1,t+1}
\\ z_{2,t+1}
\end{array} \right)$, where $z_{1,t+1}$ and $z_{2,t+1}$ are independent AR(1) --see Example \@ref(exm:GAR1)-- and
...
\\ z_{2,t+1}
\end{array} \right)$, where $z_{1,t+1}$ and $z_{2,t+1}$ are
  independent Markov
chains.

* $\left( \begin{array}{l} \lambda'_1 \\
\lambda'_2
\end{array}\right)z_{t+1}$, where $z_{t+1}$ is a Markov chai.
:::
...
## AFFINE (OR Car) PROCESSES, Building Multivariate Affine Processes {#buildingmulti}


* Recursive approach (bivariate case): $w_t = \left(\begin{array}{c}
w_{1,t}\\
w_{2,t}
\end{array}
\right)$.
...
## AFFINE (OR Car) PROCESSES, Wishart autoregressive (WAR) processes {#WAR}
...
## AFFINE (OR Car) PROCESSES, Multivariate Stochastic Parameters Processes
...
\end{array} \right)$$\left( \begin{array}{l} z_{1,t+1}
\\ z_{2,t+1}
\end{array} \right)$, where $\Lambda_1$ is a $(n \times J_1)$ matrix and $z_{1,t+1}$ is a Markov chain valued in the set of selection vectors of size $J_1$ (see Slide \@ref(slide:Markov}), $\Lambda_2$ is the same matrix as in (i) and $z_{2,t+1}$ is a Markov chain valued in the set of selection vectors of size $J_2$.

* $\left( \begin{array}{l} \Lambda_1 \\
\Lambda_2
...
## AFFINE (OR Car) PROCESSES, First Key Property: Multi-Horizon Laplace Transform
...
**Remark:** If the functions $a_{t}$ and $b_{t}$ do not depend on $t$, these recursive formulas do not depend on $t$, and we get $\varphi_{t,h}(\gamma_1,\dots,\gamma_h)$, for any $t$, with only one recursion for each $h$.
...
* Denoting by $q_t$ the price index on date $t$ and by $\pi_{t+1} =   log
...
$$
* If $w_t = (y_t, x'_t)'$ with $y_t = \log
\frac{S_t}{S_{t-1}}$, then:
$$
F(t,h) = S_t \mathbb{E}^{\mathbb{Q}}_t   \exp(e'_1 w_{t+1}+\dots+e'_1 w_{t+h}) \Rightarrow u_i = e'_1,  i=1,\dots,H.
$$
:::

:::{.example #ExponentialPayoff name="Exponential affine payoffs"}
...
## AFFINE (OR Car) PROCESSES, Second Key Property: VAR representation
...
## Multi-horizon conditional means and variances
...
\end{eqnarray}
Eq. \@ref(eq:condvar) notably shows that  $\mathbb{V}ar_t(w_{t+h})$ is an affine function of $w_t$ (indeed $\Sigma(.)$ is an affine function and the conditional expectations $\mathbb{E}_t(w_{t+h})$ are affine in $w_t$, as shown in Eq. \@ref(eq:condmean).
...
:::{.proof}
Eq. \@ref(eq:condmean) is easily deduced from Eq. \@ref(eq:VARw), using that $\mathbb{E}_t(\varepsilon_{t+k})=0$ for $k>0$.

As regards Eq. \@ref(eq:condvar):
\begin{eqnarray*}
...
## Truncated Laplace Transforms of Affine Processes and Transform Analysis {#TransfAna}
...
* We have [@Duffie_Pan_Singleton_2000]:
\begin{equation}
...
* Note that the integral in Eq. \@ref(eq:DPS) is one dimensional (whatever the dimension of $w_t$).
...
$$
(noting $w=\tilde{w}_{t+1,T}$) knowing the function $\varphi_t(z)=\mathbb{E}_t[exp(z'w)],$ $z$: complex components.\\
