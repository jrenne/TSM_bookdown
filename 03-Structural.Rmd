...
\end{equation}
To price longer-term real bonds, one can use \@ref(eq:SDF100) iteratively (which is equivalent to $\mathcal{B}_{t,h} = \mathbb{E}_t(\mathcal{M}^*_{t,t+1}\times\dots\times\mathcal{M}^*_{t+h-1,t+h})$). For instance, for $h=2$:
\begin{eqnarray*}
