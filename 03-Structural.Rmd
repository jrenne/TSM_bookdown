...
```{r RRAIESCCAPM, echo=FALSE, fig.cap="Power utility situation. Illustration of the effect of the RRA $\\gamma$ or the IES ($1/\\gamma$) on utility. The Risk Premium refers to the RRA interpretation; it can be interpreted as certainty equivalent consumption.", fig.asp = .6, out.width = "95%", fig.align = 'left-aligned', message=FALSE}
...
For sake of comparison: microeconomic studies point to estimates of $\gamma$ in $[1,3]$ (e.g., @Hartley_Lanot_Walker_2014). This constitutes the *equity premium puzzle* [@Mehra_Prescott_1985].
...
$$
The previous inequality directly results from \@ref(eq:MRCov), using the fact that $|\mathbb{C}ov(X,Y)| \le \sigma(X)\sigma(Y)$ (Cauchy-Schwarz inequality).
...
\end{equation}
but this is not true. (This would be the case under the expectation hypothesis; see Eq. \@ref(eq:stdbondRFchapterP).) To better see that, take $\mathcal{B}_{t,2}$ again; we have:
\begin{eqnarray*}
\mathcal{B}_{t,2} &=&  \mathbb{E}_t \left[ \delta \exp(-\gamma \Delta c_{t+1}) \color{blue}{\mathcal{B}_{t+1,1}} \right]\\
...
\begin{equation}
U_t = \sum_{s=t}^{\infty} \delta^{s-t} u(C_s - X_s) \quad \mbox{with} \quad X_t = \rho X_{t-1} + \lambda C_t.(\#eq:Uhabitnonstoch)
\end{equation}
...
@Epstein_Zin_1989 have proposed a framework where there is a premium for early resolution and where the time composition of risk matters. This framework has been used successfully in various asset-pricing studies.
...
* *Lottery A*: There is single draw at $t=1$. Starting at t = 1, the lottery pays either $C_h$ at all future dates (probability of 1/2), or $C_l$ at all future dates (probability of 1/2).
* *Lottery B*: In each period $t = 1, 2, \dots$, this lottery pays $C_h$ with probability 1/2 or $C_l$ with probability 1/2; the outcomes ($t = 1, 2, \dots$) are serially independent.
...
### Epstein-Zin preferences and early resolution of uncertainty
...
```{r EZresolution, echo=FALSE, fig.cap="Comparison of utilities associated with Scenarios C and D. In Scenario C, uncertainty is resolved earlier. Hence, we have a preference for early resolution of uncertainty when the measure is positive.", fig.asp = .6, out.width = "95%", fig.align = 'left-aligned', message=FALSE}
...
\end{eqnarray}
Besides, \@ref(eq:SSS) gives the following expression for the log-SDF:
\begin{eqnarray*}
...
Using that $\frac{\partial U_t}{\partial C_t}=(1 - \delta)U_t^\rho/C_t^{\rho}$ (which results from \@ref(eq:EZpreferences)), we get, using \@ref(eq:WWWWW):
\begin{equation}
...
Combining \@ref(eq:EZpreferences2), \@ref(eq:WWW) and \@ref(eq:Ra), it can be shown that:
$$
...
@Bansal_Yaron_2004 show that the combination of long-run risks and Epstein-Zin preferences allow to address several asset-pricing puzzles. This [web interface](https://jrenne.shinyapps.io/LRRModels) presents outputs of the approach. It also allows to simulate the model dynamics and to assess the influence of the parameterization.
...
```{r BY2, fig.align = 'center', out.width = "100%", fig.cap = "Source: Bansal and Yaron (2004). Panel C corresponds to the case with no long-run risk.", echo=FALSE}
