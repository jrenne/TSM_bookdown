...
c. It implies maximum Sharpe ratios that are far too low [@Hansen_Jagannathan_1991].

Using \@ref(eq:MRCov) in the power-utility situation, we get the following average excess return:
\begin{eqnarray*}
...
Because consumption is smooth, the covariance $\mathbb{C}ov_t\left(\Delta c_{t+1},R_{i,t+1}\right)$ is relatively small (see column ``$Cov(er_e,\Delta c)$'' of Table \@ref(tab:Campbell1), taken from @Campbell_1999).
...
$$
The previous inequality results from \@ref(eq:MRCov), using the fact that $\mathbb{C}ov(X,Y) \le \sigma(X)\sigma(Y)$.
...
Let's denote by $r_{t}$ the one-period risk-free real interest rate. We consider the power-utility time-separable preferences context, implying that the real SDF, between dates $t$ and $t+1$, is given by $\delta\exp(-\gamma \Delta c_{t,t+1})$. Applying \@ref(eq:SDF100) to the one-period risk-free real bond, we get:
\begin{eqnarray*}
...
\end{equation}
To price longer-term real bonds, one can use \@ref(eq:SDF100) iteratively. For instance, for $h=2$:
\begin{eqnarray*}
...
With E-Z preferences, another key correlation is that between stock returns and updates about future consumption growth---the second term in Eq. \@ref(eq:sdfAbc).
