...
In this chapter, we introduce structural approaches that have been used to investigate asset pricing. *Structural models* aim to link the stochastic discount factor (SDF) to investor risk preferences. Investors make portfolio decisions to obtain a desired time and risk profile of consumption. Loosely speaking, in structural asset-pricing models, the SDF $\mathcal{M}_{t,t+1}$ captures the aspects of utility that matter for valuing the assets.

## Consumption-based Capital Asset Pricing Model (CCAPM) and stochastic discount factor (SDF) {#PricingEquilibrium}

A seminal example of structural asset-pricing model is that of the consumption capital asset pricing model, or CCAPM (see, e.g., @Merton_1973 and @BREEDEN1979265). The CCAPM extends the CAPM framework by providing a consumption-based theory of the determinants of the valuation of the market portfolio.
...
The representative agent maximizes her expected utility time-separable preferences (see Definition \@ref(def:EUTSpref)) subject to the budget constraints at $t+j$, $j \ge 0$, see Eq. \@ref(eq:BConstr).
...
Another way to look at formula \@ref(eq:MRCov) is:
\begin{eqnarray*}
...
Consider an agent whose wealth is $W$. She will consumes only on date $1$, and features a power utility function $U$ (see Example \@ref(exm:CCAPM)). She can invest, on date $0$, in an asset whose price is 1 and whose payoff is $1+\varepsilon$ with probability $1/2$ and $1/(1+\varepsilon)$ with probability $1/2$.
...
### The limitations of the CCAPM

Three important limitations of the CCAPM approach have been largely documented in the literature:

* Fitting average excess return implies implausible risk aversions (*equity premium puzzle*).
...
Using Eq. \@ref(eq:MRCov) in the power-utility situation, we get the following average excess return:
\begin{eqnarray*}
...
A third problem pertains to the volatility of the SDF. @Grossman_Shiller_1981 and @Hansen_Jagannathan_1991 show that observed Sharpe ratios give lower bounds to the volatility of SDF:
$$
...
$$
The previous inequality results from Eq. \@ref(eq:MRCov), using the fact that $\mathbb{C}ov(X,Y) \le \sigma(X)\sigma(Y)$.
...
@Hansen_Singleton_1982 have developed and used the General Method of Moments to test the C-CAPM. This approach is based on Eq. \@ref(eq:pricingcapm):
\begin{equation}
...
* The decrease in bond term premiums over the last decades is consistent with decrease in the correlation between long-term bond excess returns and consumption (see Figure \@ref(fig:fredCorrel), and this [web interface](https://jrenne.shinyapps.io/APModels)).
...
### The term structure of real interest rates in the CCAPM
...
Applying \@ref(eq:SDF100) to the one-period risk-free real bond, using that the real SDF, between dates $t$ and $t+1$, is given by $\delta\exp(-\gamma \Delta c_{t,t+1})$, we get:
\begin{eqnarray*}
...
\begin{equation}
r_{f,t} = \underbrace{- \log(\delta) + \gamma \mu (1-\phi)  - \frac{\gamma^2\sigma_c^2}{2}}_{=\eta_0} + \underbrace{\gamma \phi}_{=\eta_1} g_t.(\#eq:rCCAPM)
\end{equation}
To price longer-term real bonds, one can use Eq. \@ref(eq:SDF100) iteratively. For instance, for $h=2$:
\begin{eqnarray*}
...
Further, for $h>1$:
\begin{equation}
P_{t,h} = \mathbb{E}_t \left[ \delta^h \exp(-\gamma \Delta c_{t+1} - \gamma \Delta c_{t+2} \dots - \gamma \Delta c_{t+h}) \right].(\#eq:PthCCAPM)
\end{equation}
It is then tempting to say that this price is also equal to
\begin{equation}
\mathbb{E}_t \left[ \exp(- r_{f,t} - r_{f,t+1} - \dots - r_{f,t+h-1}) \right],(\#eq:CCAPMEH)
\end{equation}
...
Exploiting the fact that the state variable ($g_t$) follows an affine process, Proposition \@ref(prp:reverseMLT) can be used to recursively compute the bond prices given in \@ref(eq:PthCCAPM). These prices are exponential affine in $g_t$, and the real rates are therefore affine functions of $g_t$.

In order to compute the counterfactual yields that would prevail under the Expectation Hypothesis, we also make use of the Proposition \@ref(prp:reverseMLT). Specifically, we have (from Eq. \@ref(eq:CCAPMEH)):
\begin{eqnarray}
...
\end{eqnarray}
where $\eta_0$ and $\eta_1$ are defined in \@ref(eq:rCCAPM).

<!-- ```{r TSMCCAPM, echo=FALSE, fig.cap="Term structures of real rates in a small dynamic CCAPM. The parameters are as follows: $\\phi=0.6$, $\\mu = 0.005 (1 - \\phi)$, $\\gamma=10$, $\\delta = 0.99$, $\\sigma_c=0.008$.", fig.asp = .6, out.width = "95%", fig.align = 'left-aligned', message=FALSE} -->
<!-- library(AEC) -->
...
<!-- mu.star <- mu * (1 - phi) - sigma.c * lambda.0 -->

<!-- Model <- list() -->
<!-- Model$phi <- phi -->
<!-- Model$mu  <- mu * (1 - phi) -->
<!-- Model$Sigma <- sigma.c -->
<!-- Model$delta.0 <- -log(beta) + mu * gamma * (1 - phi) - gamma^2 * sigma.c^2 / 2 -->
<!-- Model$delta.1 <- gamma * phi -->
...
<!-- r.bar <- Model$delta.0 + Model$delta.1 * mu -->

<!-- # par(mfrow=c(1,1)) -->
<!-- # plot(t(all.A + all.B * r.bar),type="l",lwd=2) -->

<!-- # ALTERNATIVE WAY TO PRICE BONDS -->
<!-- library(TSModels) -->
...
<!-- a.h <- -c(AB$A)/(1:h) -->
<!-- b.h <- - log(beta) + -c(AB$B)/(1:h) -->

<!-- T <- 100 -->
<!-- nb.sim <- 1 -->
<!-- x.sim <- sim.arma(mu,phi,theta=c(1),sqrt(sigma2),T,mu/(1-phi),nb.sim) -->
<!-- i.t <- (Model$delta.0 + Model$delta.1 * x.sim) -->

<!-- par(mfrow=c(1,1)) -->
<!-- par(plt=c(.1,.95,.2,.95)) -->

<!-- plot(i.t,type="l",xlab="time",ylab="",ylim=c(min(i.t),max(i.t)+.02)) -->

<!-- for(t in seq(1,T,by=10)){ -->
<!--   lines(t+(1:h)-1,all.A + all.B * x.sim[t],col="red",lwd=2) -->
<!--   lines(t+(1:h)-1,all.A.H + all.B.H * x.sim[t],col="blue",lwd=2) -->
<!-- } -->
<!-- legend("topright", -->
<!--        c( -->
<!--          "Short-term rate", -->
...
eta.0 <- -log(delta) + mu * gamma * (1 - phi) - gamma^2 * sigma.c^2 / 2
eta.1 <- gamma * phi
# Specify the model:
psi.parameterization <- list(
  mu = matrix(mu * (1-phi)),
...
# Simulate growth:
x.sim <- sim.arma(mu*(1-phi),phi,theta=c(1),sigma.c,T=100,mu,nb.sim=1)
r.t   <- (eta.0 + eta.1 * x.sim)
# Prepare plots:
par(mfrow=c(1,1));par(plt=c(.1,.95,.2,.95))
plot(r.t,type="l",xlab="time",ylab="",ylim=c(min(r.t),max(r.t)+.02),las=1)
for(t in seq(1,T,by=10)){
...
&& + \gamma \phi g_t + \phi_\pi \pi_{t}. (\#eq:nominalCCAPM)
\end{eqnarray}
...
eta.0.i <- -log(delta) + mu * gamma * (1 - phi) + mu_pi * (1 - phi_pi) -
  (gamma*sigma.c + sigma.pi.c)^2/2 - sigma.pi^2/2
eta.1.i <- matrix(c(gamma * phi,phi_pi),2,1)
# Specify the model:
psi.parameterization <- list(
  mu = matrix(c(mu*(1-phi),mu_pi*(1-phi_pi)),2,1),
  Phi = diag(c(phi,phi_pi)),
  Sigma = matrix(c(sigma.c^2,sigma.c*sigma.pi.c,
                   sigma.c*sigma.pi.c,sigma.pi^2+sigma.pi.c^2),2,2))
# Use reverse-order multi-horizon LT to price ZC bonds:
...
AB <- reverse.MHLT(psi.GaussianVAR,u1,u2,H=h,psi.parameterization)
a.h.i.EH <- -(matrix(AB$A,2,h)-matrix(eta.1.i,2,h))/t(matrix((1:h),h,2))
...
plot(c(avg.i.h),type="l",lwd=2,col="red",lty=3,ylim=c(0.08,max(avg.i.h)),
...
If one assumes that $X_t$ is exogenous---a case referred to as *external habits*---and if $u(Z)=Z^{1-\gamma}/(1-\gamma)$ (Def. \@ref(def:power)) then:^[Without the external habit assumption, the Euler equation (equilibrium relationship between risk-free short-term rate and marginal utilities) is far less tractable:
\begin{eqnarray*}
...
Before looking into it, Example \@ref(exm:EZpricingInfo) explores an important implication of \@ref(eq:eq:SSS), namely the fact that, with Epstein-Zin preferences, changes in the information regarding future consumption path is priced (in the sense that it results in specific risk premiums).

:::{.example #EZpricingInfo name="Pricing information on future consumption path"}

Consider the case where $\rho = 1$ and log-normal conditionally homoskedastic consumption [@Cochrane_2005]. Using $v_t = \log(U_t)$, we have:
$$
...
&=& - \gamma(\mathbb{E}_{t+1} - \mathbb{E}_t)\Delta c_{t+1} +    (1 - \gamma) \times \underbrace{ (\mathbb{E}_{t+1} - \mathbb{E}_t) \left(  \sum_{j=1}^{\infty}  \delta^j \Delta c_{t+1+j}  \right).}_{\mbox{innovat. in long-run consump. growth}}(\#eq:sdfAbc)
\end{eqnarray}
...
Using Eqs. \@ref(eq:EZpreferences2) and \@ref(eq:WWW), it can be shown that:
$$
...
> The approximation underlying Eq. \@ref(eq:approxRa) is the same as that used by @Campbell_Shiller_1988 used to linearize stock returns.
...
```{r SPF10yrStock, echo=FALSE, fig.cap="Sources: SPF Philadelphia and Wilshire 5000 Price Index (FRED database).", fig.asp = .6, out.width = "95%", fig.align = 'left-aligned', message=FALSE,warning = FALSE}
library(fredr)
fredr_set_key("df65e14c054697a52b4511e77fcfa1f3")
start_date <- as.Date("1959-01-01"); end_date <- as.Date("2023-10-01")
f <- function(ticker){
...
$\mu + x_t$ and $\mu_d + \phi x_t$ are, respectively, the conditional expectations of consumption growth ($g_t$) and dividend growth ($g_{d,t}$). Processes $g_t$ and $g_{d,t}$ are exogenous. The model is solved by finding associated processes $r_{a,t}$ and $r_{m,t}$ that make the model internally consistent. These returns have to satisfy both Eqs. \@ref(eq:approxRa) and \@ref(eq:sdfRa).
...
* inject $r_{a,t+1}$ in Eq. \@ref(eq:sdfRa) and look for the values of $A_0$ and $A_1$ that satisfy this Euler equation.
...
As explained above, together, \@ref(eq:sdfEZ) and \@ref(eq:approxRa) imply an exponential affine SDF. This open the door to tractable term-structure modeling (see, e.g., Subsection \@ref(MHLT) and  Section \@ref(TSM)). This is exploited, e.g., by @Bansal_Shaliastovich_2013 (see Example \@ref(exm:BS2013)).
...
plot(freq*eta.0.nom,type="l",lwd=2,ylim=c(0,freq*max(eta.0.nom)),las=1,
     xlab="maturity",ylab = "rates",las=1);grid()
...
## Preferred-habitat model of the term-structure of interest rate
...
\begin{equation}
y_{n,t} = - \frac{1}{n}\log P_{n,t}.(\#eq:Pandy)
\end{equation}

:::{.proposition #prop111 name="Links between bond prices"}
...
\begin{eqnarray}
y_{1,t} &=& \mathbb{E}_t(r_{n,t,t+1}) - \frac{\gamma}{2} \frac{\partial}{\partial z_{n,t}} \mathbb{V}ar_t(r_{t,t+1}). (\#eq:Lagr1)
\end{eqnarray}
...
\begin{eqnarray}
s_{n,t} &=& (\alpha_n + {\beta_n}' f_t) - \zeta_n y_{n,t}. (\#eq:supply1)
\end{eqnarray}
:::
