...
Note however that swap yields are usually not zero-coupon yields. They are usually homogeneous to par yields, as defined in \@ref(eq:paryields) (see also the remark at the end of Subsection \@ref(Swaps)). To obtain zero-coupon from the par yield curve, one can employ an approach called *bootstrapping*. To simplify the exposition, we consider the yearly frequency. It proceeds recursively across maturities, using \@ref(eq:paryields):
