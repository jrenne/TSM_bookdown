...
\urlstyle{same}
\hypersetup{
  pdftitle={Asset Pricing with Discrete-Time Affine Processes},
...
At that stage, let us introduce the definition of two metrics that characterize agents' preferences regarding risky income (Def. \ref{def:RAmeasures}) and intertemporal substitution (Def. \ref{def:IES}).
...
\begin{figure}
\includegraphics[width=0.95\linewidth]{TSM_files/figure-latex/RRAIESCCAPM-1} \caption{Power utility situation. Illustration of the effect of the RRA $\gamma$ or the IES ($1/\gamma$) on utility. The Risk Premium refers to the RRA interpretation; it can be interpreted as certainty equivalent consumption.}\label{fig:RRAIESCCAPM}
\end{figure}
...
The CCAPM is a simple model; it is easy to test once a form for \(u\) has been posited. The next subsection shows, however, that it is difficult to reconcile with the data.
...
For sake of comparison: microeconomic studies point to estimates of \(\gamma\) in \([1,3]\) (e.g., \citet{Hartley_Lanot_Walker_2014}). This constitutes the \emph{equity premium puzzle} \citep{Mehra_Prescott_1985}.
...
\]
The previous inequality directly results from \eqref{eq:MRCov}, using the fact that \(|\mathbb{C}ov(X,Y)| \le \sigma(X)\sigma(Y)\) (Cauchy-Schwarz inequality).
...
\end{equation}
but this is not true. (This would be the case under the expectation hypothesis; see Eq. \eqref{eq:stdbondRFchapterP}.) To better see that, take \(\mathcal{B}_{t,2}\) again; we have:
\begin{eqnarray*}
\mathcal{B}_{t,2} &=&  \mathbb{E}_t \left[ \delta \exp(-\gamma \Delta c_{t+1}) \color{blue}{\mathcal{B}_{t+1,1}} \right]\\
...
\begin{quote}
One can use the risk-neutral dynamics of \(w_t\) to compute the nominal yield curve, using that \(i_{t,h} = - \log \mathbb{E}^{\mathbb{Q}}_t [\exp(-i_t - \dots - i_{t+h-1})]\). (This should give the same results as before.)
\end{quote}
...
\begin{equation}
U_t = \sum_{s=t}^{\infty} \delta^{s-t} u(C_s - X_s) \quad \mbox{with} \quad X_t = \rho X_{t-1} + \lambda C_t.\label{eq:Uhabitnonstoch}
\end{equation}
...
\]
For each of these two rate sequences, we compute the resulting sequence of consumption, with \(C_1=1\).
...
\citet{Epstein_Zin_1989} have proposed a framework where there is a premium for early resolution and where the time composition of risk matters. This framework has been used successfully in various asset-pricing studies.
...
What is the certainty equivalent \(\widetilde{C}_1\) to the period-1 gamble? \(\widetilde{C}_1\) solves:
\[
U_0 = \left((1-\delta)C_0^{1-\rho} + \delta (1-\delta) \widetilde{C}_1^{1-\rho}\right)^{\frac{1}{1-\rho}},
\]
that is:
\[
\widetilde{C}_1 = \left(\frac{1}{2}C_h^{1-\gamma}+\frac{1}{2}C_l^{1-\gamma}\right)^{\frac{1}{1-\gamma}}.
\]
This certainty equivalent is approximately the same as the one that one would get if the utility function was the standard (time-separable) power utility function (with RRA \(= \gamma\)).
...
\tightlist
\item
  \emph{Lottery A}: There is single draw at \(t=1\). Starting at t = 1, the lottery pays either \(C_h\) at all future dates (probability of 1/2), or \(C_l\) at all future dates (probability of 1/2).
\item
  \emph{Lottery B}: In each period \(t = 1, 2, \dots\), this lottery pays \(C_h\) with probability 1/2 or \(C_l\) with probability 1/2; the outcomes (\(t = 1, 2, \dots\)) are serially independent.
\end{itemize}
...
However, with time-separable utility functions, agents would be indifferent between playing the two lotteries. The reason is that the time-separable model evaluates risks at different dates in isolation \citep{Piazzesi_Schneider_2007}. From the perspective of time zero, random consumption at any given date---viewed in isolation---does have the same risk (measured, for example, by the variance.) That is in this sense that for Epstein-Zin preferences (and other recursive preference schemes), we say that the \emph{time-composition of risk} matters.

Let's first consider Lottery A. At date \(t=1\), there is no uncertainty any more. It is easily seen that, if one draws \(C_i\) (\(i \in \{l,h\}\)) at date 1, then \(U_1^A = C_i\). Hence, as of date \(t=0\), we have:
\begin{equation}
...
\hypertarget{epstein-zin-preferences-and-early-resolution-of-uncertainty}{%
\subsection{Epstein-Zin preferences and early resolution of uncertainty}\label{epstein-zin-preferences-and-early-resolution-of-uncertainty}}

Consider two new lotteries: \emph{Lottery C} and \emph{Lottery D}; three periods are involved: 0, 1 and 2. The consumption of dates 1 and 2 are determined by independent tosses of a fair coin: either \(C_l\) or \(C_h\). The difference between Lotteries C and D pertains to the date on which the information about the tosses is revealed: in Lottery C, the outcomes of the two tosses are revealed at date 1. in Lottery D, the second toss is not revealed before date \(2\).
...
\end{eqnarray}
Besides, \eqref{eq:SSS} gives the following expression for the log-SDF:
\begin{eqnarray*}
...
At date \(t=1\), one gets information about future consumption levels:
...
Using that \(\frac{\partial U_t}{\partial C_t}=(1 - \delta)U_t^\rho/C_t^{\rho}\) (which results from \eqref{eq:EZpreferences}), we get, using \eqref{eq:WWWWW}:
\begin{equation}
...
Combining \eqref{eq:EZpreferences2}, \eqref{eq:WWW} and \eqref{eq:Ra}, it can be shown that:
\[
...
Recall that, for any asset \(i\), whose return is \(1+R_{i,t+1}\), we must have:
\begin{equation}
1 = \mathbb{E}_t(\mathcal{M}_{t,t+1}(1+R_{i,t+1})). \quad \mbox{(Euler equation)}\label{eq:Euler}
\end{equation}
This should apply, in particular, to the wealth portfolio. That is, for \(1+R_{i,t+1} = 1+R_{a,t+1} = \exp(r_{a,t+1})\), we get:
\begin{equation}
...
We have shown that the CCAPM had difficulties in generating large average excess returns without implying unreasonable risk aversion parameters. Epstein-Zin preferences can address this problem.
...
For this second channel to be relevant, observed excess returns should positively correlate to the updates of expectations of long-term consumption. Figure \ref{fig:SPF10yrStock} offers simple empirical evidence for this.

\begin{figure}
\includegraphics[width=0.95\linewidth]{TSM_files/figure-latex/SPF10yrStock-1} \caption{Sources: SPF Philadelphia and OECD for share price index (ticker: SPASTT01USM661N in FRED).}\label{fig:SPF10yrStock}
\end{figure}

\citet{Bansal_Yaron_2004} show that the combination of long-run risks and Epstein-Zin preferences allow to address several asset-pricing puzzles. This \href{https://jrenne.shinyapps.io/LRRModels}{web interface} presents outputs of the approach. It also allows to simulate the model dynamics and to assess the influence of the parameterization.
...
\caption{Source: Bansal and Yaron (2004). Panel C corresponds to the case with no long-run risk.}\label{fig:BY2}
\end{figure}
...
\]
Up to Jensen's inequality, and taking expectations on both sides, we get:
\[
...
\]
The binary variable \(\ell_t\) is assumed not to Granger-cause \(y_t\) (as in Hypothesis \ref{hyp:A1}) and not to be priced (as in Hypothesis \ref{hyp:A3}). As in Proposition \ref{prp:defaultBondclassical}---and more precisely equation \eqref{eq:lambdaPQ}---this implies that
\[
...
When hit by a liquidity shock, a bondholder has to liquidate her bond in a small amount of time (between \(t\) and \(t+\varepsilon\), with \(\varepsilon \ll 1\)). She collects a random number (\(\kappa\)) of offers on the market. This number follows a Poisson distribution of parameter \(\varepsilon\gamma^{(e)}\); the more liquid the bond, the higher \(\gamma^{(e)}\). Each offer is a random fraction \(\omega_{i}\) (\(i\in\{1,\ldots,\kappa\}\)) of \(\widetilde{B}^{(e)}_{t,h}\) (see Eq. \eqref{eq:pseudopriceilliq}), where the \(\omega_{i}\)'s are independently and uniformly distributed in \([0,1]\).
...
\begin{verbatim}
##           [,1]       [,2]       [,3]       [,4]       [,5]      [,6]       [,7]
## [1,] 0.9459864 0.89814451 0.85547299 0.81714944 0.78249098 0.7509389 0.72203656
## [2,] 0.0555271 0.05371215 0.05203358 0.05048332 0.04905458 0.0477385 0.04652564
##            [,8]       [,9]     [,10]
## [1,] 0.69540915 0.67074736 0.6477943
## [2,] 0.04540686 0.04437364 0.0434182
\end{verbatim}
