...
Though the approach is then not necessarily optimal, it shows good empirical properties (\citet{deJong_2000} or \citet{Duan_Simonato_1999}). In order to test for the validity of the approach in a specific context, one can resort to Monte-Carlo simulations \citep{zarg_2017}.

\hypertarget{nonlinear}{%
\section{Non-linear models}\label{nonlinear}}

As soon as \(w_t\) follows an affine process, it admits the VAR dynamics presented in Prop. \ref{prp:affineVAR}, i.e., it features a linear transition equation. However, measurement equations may be non-linear (affine) functions of \(w_t\). This is in particular the case if observed variables include Swaps rates (see Remark SWAPS XXXX), CDS rates (see Subsection \ref{subsec:Credit:CDS}, in particular Eq. \eqref{eq:MCCDSformula1}) or prices of tranche products (see Example \ref{exm:DD}).
...
This approach works under the assumption that some of the observed variables are \emph{perfectly priced} (or modelled).(Recall that \(y_t\) and \(w_t\) are respectively of dimension \(m\) and \(n\), see Eqs. \eqref{eq:measeq} and \eqref{eq:transeq} in Def. \ref{def:LSSM}.) Formally:
...
\end{equation}
where \(\varepsilon_t|\underline{x_t},z_t \sim \mathcal{N}(0,\Sigma(z_t))\) and if \(z_t\) is an exogenous independent Markov chain, then \(w_t = (x_t',z_t')'\) is affine. Using the notations of Eq. \eqref{eq:dynxRS}, we have:
\[
m(z_t,x_{t-1}) = \mu z_t + \Phi x_{t-1}.
\]
If \(r_t\) and the s.d.f. are respectively affine and exponential in \(w_t\), then, in particular, yields are also affine in \(w_t\), i.e.~of the form \(R(t,h)= A_h'z_t + B_{h}'x_t\) (see Eq. \eqref{eq:RthAB}). Therefore, if the components of \(y_t\) are yields of different maturities, the measurement equations are consistent with Assumption \ref{hyp:RSmeasurement}.
\end{example}

How to estimate such a model when the regimes \(z_t\) are unobservable? We distinguish two distinct situations:

\textbf{Case 1. The \(x_t\) factors are observable.} The probabilities of being in the different regimes on each date can be estimated by employing the Kitagawa-Hamilton filter, with (using the notations of Proposition \ref{app:KitagHamilton}): \(F_t = (y_t',x_t')'\) and
\begin{eqnarray*}
...
\end{eqnarray*}
where \(\mathbf{n}(u;\mu,\Omega)\) denotes the evaluation, at vector \(u\) of the p.d.f. of the multivariate normal distribution \(\mathcal{N}(\mu,\Omega)\) (see eq. \eqref{eq:varPHI}). A by-product of the Kitagawa-Hamilton filter is the likelihood function associated with the dataset. As a result, the model parameterization can be estimated by maximum llikelihood approach.
...
\hypertarget{credit-and-liquidity-risks}{%
\chapter{Credit and liquidity risks}\label{credit-and-liquidity-risks}}
...
and
\begin{equation}
\boxed{B(t,h) = \mathbb{E}_t^{\mathbb{Q}}\left[\exp\left(-r_t-\dots-r_{t+h-1}- \lambda_{t+1} - \dots - \lambda_{t+h}\right)\right].}\label{eq:standardPriceCredit2}
\end{equation}
which rewrites, using \(\tilde{r}_t = r_t + \lambda_{t+1}\):
\begin{equation}
B(t,h) = \mathbb{E}_t^{\mathbb{Q}}\left[\exp\left(-\tilde{r}_t-\dots-\tilde{r}_{t+h-1}\right)\right],\label{eq:standardPriceCredit3}
\end{equation}
\end{proposition}

\begin{proof}
We have (Eq. \eqref{eq:fQfP}):
\begin{equation}
f^{\mathbb{Q}}(d_t,y_t|d_{t-1},y_{t-1}) = \exp(r_{t-1})\mathcal{M}_{t-1,t} f^{\mathbb{P}}(d_t,y_t|d_{t-1},y_{t-1}).\label{eq:aaq1}
\end{equation}
...
&=& \exp(r_{t-1})\mathcal{M}_{t-1,t} \underbrace{f^{\mathbb{P}}(y_t|y_{t-1})}_{\mbox{by hypothesis}}. \label{eq:aaq2}
\end{eqnarray}
By Bayes, we have:
\[
f^{\mathbb{Q}}(d_t|d_{t-1},y_t,y_{t-1}) = \frac{f^{\mathbb{Q}}(d_t,y_t|d_{t-1},y_{t-1})}{f^{\mathbb{Q}}(y_t|d_{t-1},y_{t-1})}.
\]
Using Eq. \eqref{eq:aaq1} (numerator) and Eq. \eqref{eq:aaq2} (denominator), we get:
\begin{eqnarray*}
...
As a result:
\begin{equation}
B(t,h) = \mathbb{E}_t^{\mathbb{Q}}\left[\exp\left(-r_t-\dots-r_{t+h-1}- \lambda_{t+1}^{\mathbb{Q}} - \dots - \lambda_{t+h}^{\mathbb{Q}}\right)\right].\label{eq:standardPriceCredit}
\end{equation}
...
\begin{quote}
Eq. \eqref{eq:standardPriceCredit3} is reminiscent of Eq. \eqref{eq:stdbond}, where the risk-free short-term rate \(r_t\) is replaced by the \emph{credit-adjusted short-term rate} \(\tilde{r}_t\).
\end{quote}

Importantly, if both \(r_t\) and \(\lambda_t\) are affine in \(y_t\), where the latter is an affine process, then bond prices are easily computed by means of recursive formulas (as was the case in the context of Eq. \eqref{eq:stdbond}, see Example \ref{exm:nominalBth}.

In this case, \(B(t,h)\) is exponential affine in \(y_t\), that is, it is of the form:
\begin{equation}
B(t,h) = \exp({A^d_h}'y_t + B^d_h)\quad (say).\label{eq:standardPriceCredit4}
\end{equation}
...
\protect\hypertarget{prp:RMVclassical}{}\label{prp:RMVclassical}Under Hypotheses \ref{hyp:A1} and \ref{hyp:A3}, and if, in case of default on date \(t+i\), the recovery payment is \(\zeta \widetilde{B}(t+i,h-i)\), with \(\zeta \in [0,1]\), then
\begin{equation}
B(t,h) =  (1-d_t)\widetilde{B}(t,h),\label{eq:BBcredit}
\end{equation}
where \(\widetilde{B}(t,h)\) is a \emph{pseudo-price} given by:
\begin{equation}
\widetilde{B}(t,h) = \mathbb{E}_t^{\mathbb{Q}} [\exp(-r_{t}-\tilde\lambda_{t+1}-\dots -r_{t+h-1}-\tilde\lambda_{t+h}],\label{eq:BtildeCredit}
\end{equation}
...
\begin{proof}
Relation \eqref{eq:BBcredit} is true for \(B(t+h, 0)\) since \(\widetilde{B}(t+h, 0) = 1\) and \(B(t+h, 0) = 1 - d_{t+h}\).
...
\begin{equation}
B(t, h) = \left(1 - d_{t} \right)\exp(- h[\omega_0 + \kappa_{0}] - \omega_{1} ' y_{t} + A_h'y_t + B_h),\label{eq:standardAffineCredit}
\end{equation}
...
\item
  In an affine framework, the computation of \(\widetilde{B}_1 (t, h)\) (and \(B_1 (t, h)\)) is straightforward;
\item
  but formulas for \(\widetilde{B}_2 (t, h)\) and \(B_2 (t, h)\) are not explicit anymore even if \(\widetilde{\lambda}_{2, t}\) is affine in \((y_t, y_{t-1}, d_{1, t}, d_{1, t-1})\).
\end{enumerate}
...
\]
The binary variable \(\ell_t\) is assumed not to Granger-cause \(y_t\) (as in Hypothesis \ref{hyp:A1}) and not to be priced (as in Hypothesis \ref{hyp:A3}). As in Proposition \ref{prp:defaultBondclassical} (and more precisely Eq. \eqref{eq:lambdaPQ}), this implies that
\[
...
In that context, following the approach used in Proposition \ref{prp:RMVclassical}, one can define a \emph{pseudo-illiquidity intensity} as follows (mimicking Eq. \eqref{eq:pseudodefintens}):
\begin{eqnarray}
...
\begin{equation}
\widetilde{B}_e (t, h) = \mathbb{E}^{\mathbb{Q}}_t \left\{ \exp \left[ - \sum_{i=1}^{h} \left( r_{t+i-1} + \widetilde{\lambda}_{e,c, t+i} + \widetilde{\lambda}_{e,\ell, t+i} \right) \right] \right\}.\label{eq:pseudopriceilliq}
\end{equation}
...
When hit by a liquidity shock, a bondholder has to liquidate her bond in a small amount of time (between \(t\) and \(t+\varepsilon\), with \(\varepsilon \ll 1\)). She collects offers on the market; each offer is a random fraction \(\omega_{i}\) (\(i\in\{1,\ldots,\kappa\}\)) of \(\widetilde{B}_e (t, h)\) (see Eq. \eqref{eq:pseudopriceilliq}), where the \(\omega_{i}\)'s are uniformly distributed in \([0,1]\).
...
\citet{DUBECQ201629} decompose the EURIBOR versus Overnight Index Swap (OIS) spreads into a credit and a liquidity component. (See Subsection \ref{Swap} for the definition of a swap contract.)
...
\begin{equation}
R^{IBOR}_{t,h} = - \frac{1}{h} \log \mathbb{E}^{\mathbb{Q}}_t \left\{ \exp \left[ - \sum_{i=1}^{h} \left( r_{t+i-1} + \widetilde{\lambda}_{e,c, t+i} + \widetilde{\lambda}_{j,\ell, t+i} \right) \right] \right\}.\label{eq:EURIBOR}
\end{equation}
...
\begin{equation}
R^{OIS}_{t,h} = - \frac{1}{h} \log \mathbb{E}^{\mathbb{Q}}_t \left\{ \exp \left( - \sum_{i=1}^{h}  r_{t+i-1} \right) \right\}.\label{eq:OIS}
\end{equation}
...
\item
  ...
  \item
    Eq. \eqref{eq:standardPriceCredit2} is no longer valid for entity \(e = 1\). It remains valid for \(e=2\) but the computation of \(\widetilde{B}_{2} (t, h)\) is not straightforward: even if \(y_t\) is \(\mathbb{P}\)-autonomous, this is not true under \(\mathbb{Q}\).
  \end{itemize}
\end{itemize}
...
\protect\hypertarget{hyp:H4}{}\label{hyp:H4}The one-period positive SDF \(\mathcal{M}_{t-1, t}\) is given by (this is Eq. \eqref{eq:keySDF}, with \(\alpha_t \equiv \alpha_w\)):
\begin{equation}
...
\end{array}
\end{equation}
\(\alpha_w = (\alpha_y ' , \alpha_x ' , \alpha_{\delta} ') '\) is the vector of \emph{prices of risk}.
\end{hypothesis}
...
\mathbb{E}^{\mathbb{Q}} \left\{ \exp \left[ - \sum_{j = i}^{h-1} \left(r_{t+j} + \delta^{(e)}_{t+j+1} \right) \right]  \Big|  \underline{w_{t+i}} \right\},\label{eq:ZCBRMV1}
\end{equation}
with \(RR_t^{(e)} = \exp(-\delta_t^{(e)})\), and under Hypothesis \ref{hyp:H1} to \ref{hyp:H4}, the price \(B_{e} (t, h)\) at date \(t < \tau^{(e)}\) is given by:
\begin{equation}
B_{e} (t, h) = \mathcal{V}^{(e)}_{t, h},\label{eq:ZCBRMV2}
\end{equation}
where
\begin{equation}
\begin{array}{lll}
...
\begin{equation}
\boxed{B_{e} (t, h) = \mathbb{E}^{\mathbb{Q}} \left\{ \exp \left[ -  \left( \sum_{i = 1}^{h}{\color{red}r_{t+i-1} + \delta^{(e)}_{t+i}} \right) \right]  \Big|  \underline{w_{t}} \right\}.}\label{eq:ZCBRMVformula2}
\end{equation}
It is reminiscent of Eq. \eqref{eq:standardPriceCredit2}, with \(\lambda\) (default intensity) replaced with \(\delta\) (credit-event variable). However, contrary to the classical framework, credit events are priced sources of risk, the no-jump condition is relaxed, contagion is allowed, and the recovery rate is stochastic. This formula can therefore be seen as a generalization of the setting of \citet{Duffie_Singleton_1999}.
...
\begin{proof}
See Online Appendix A.4 of \citet{Monfort_Pegoraro_Renne_Roussellet_2021}. The proof notably makes use of Lemma \ref{lemma:petitlemme} (Subsection \ref{AffineLaplace}).
\end{proof}
...
Building on \citet{Gourieroux_Monfort_Renne_2014}, \citet{Gourieroux_Monfort_Mouabbi_Renne_2021} propose an affine top-down model consistent with: the presence of systemic entities, contagion, and the pricing of default events.

\begin{example}[Disastrous Defaults]
\protect\hypertarget{exm:DD}{}\label{exm:DD}\leavevmode

\begin{itemize}
\tightlist
\item
  Model proposed by \citet{Gourieroux_Monfort_Mouabbi_Renne_2021}.
\item
...
\item
  \((y_t,x_t)'\) follows a VARG dynamics (Eq. \eqref{eq:VARG}). This dynamics admits the following VAR representation (using Prop. \ref{prp:affineVAR}):
  \begin{equation}
  \left\{
  \begin{array}{ccl}
...
\item
  If \(c>0\):
  Defaults on date \(t\) increases the conditional probability of having additional defaults on the next date \(\Rightarrow\) Systemic defaults are infectious \citep{Davis_Lo_2001}, or contagious.
\item
  ...
  \end{equation}
\item
  \(\gamma_0\) is a distribution featuring a point mass at zero {[}see Eq. (\eqref{eq:ARG0} in Example \ref{exm:ARG1}{]}.
\end{itemize}
...
\tightlist
\item
  If \(\mu_{c,z}<0\) and \(|\mu_{c,z}|\) is large or if \(\mu_{c,z}<0\) and \(|\mu_{c,z}|\) not so large but \(c\) (contamination) is large, then \textbf{systemic defaults can give rise to disastrous decreases in \(C_t\)}.
\item
  Agents feature Epstein-Zin preferences, with a unit elasticity of intertemporal substitution (EIS). An admissible s.d.f. is exponential affine {[}as in Eq. \eqref{eq:keySDF}{]}.
\end{itemize}
...
\item
  Pricing formula based on an intensive use of the truncated Laplace transform \citep[see Eq. \eqref{eq:DPS},][]{Duffie_Pan_Singleton_2000}.
\item
